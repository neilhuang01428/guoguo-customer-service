...
# 用於 Cloudflare Pages —— Build command: bash scripts/build-neutral.sh ／ Output dir: dist
#
# 原理：導外資訊（回賣場/LINE/電話…）只在導外版由 Worker 注入，原始檔天生乾淨；
#       這裡再用 allowlist 只挑乾淨檔、加 noindex、並用洩漏檢查當最後守門。
# ─────────────────────────────────────────────────────────────
set -euo pipefail
cd "$(dirname "$0")/.."   # 腳本在 scripts/ → 回 repo 根操作 articles/、dist/、assets/、tag/
...
#    文章都在 articles/<slug>/ 底下 → 整個 articles/ 資料夾複製進 dist（網址深一層：/articles/<slug>/）。
#    新增文章只要進 articles.json（並在 articles/ 建好資料夾、重跑 build-homepage.py），這裡與麵包屑注入都會自動跟上。
...
if [ -d articles ]; then
  cp -R articles "$OUT"/                    # 整包搬進 dist/articles/
else
  echo "❌ 找不到 articles/ 資料夾 → 中止（文章應都在 articles/<slug>/ 底下）。"
  exit 1
fi
# 逐 slug 確認 articles/<slug>/ 存在（articles.json 列了卻沒建 → 首頁會連到 404，先擋下）
for d in $ARTICLE_SLUGS; do
  if [ ! -d "articles/$d" ]; then
    echo "❌ articles.json 列了 slug「$d」，卻找不到 articles/$d → 中止（首頁會連到 404，請先建好資料夾）。"
    exit 1
  fi
done
...
def tags_html(slug):
    ...
    if not tags:
        return ""
    # 連到「靜態標籤頁」（相對 ../../tag/<slug>/）：文章在中性版 /articles/<art>/ → /tag/<slug>/
    # （文章深一層，故 ../../）。用相對路徑，同一份注入邏輯在導外（Worker）與中性都對。
    chips = "".join(
